...
glob(path + '*.json', {}, async (err, files) => {
  ...
  for(j=0; j<i; j++) {
    for(k=j+1; k<i; k++) {
      if(products[j]["name"] == products[k]["name"] ||
         (products[j].hasOwnProperty("repositoryURL") && products[j].hasOwnProperty("repositoryURL") &&
          products[j]['repositoryURL'] == products[k]['repositoryURL'])){
        console.log('Duplicate found between ' + products[j]['filename'] + " and " + products[k]['filename'])
        dups += 1
      }
    }
  }

  console.log(dups + " duplicates found based on either name or repositoryURL.")
})